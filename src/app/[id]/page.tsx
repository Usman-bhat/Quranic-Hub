import { data } from './data'; // Adjust the import path based on your project structure 
import SurahCard from "@/components/Chapter/SurahCard";
import SizeChanger from "@/components/Chapter/SizeChanger";
import Navbar from "@/components/Navbar";
// ... (your existing imports)

export async function generateStaticParams() {
  return data.map((item) => ({
    id: item.id.toString()
  }));
}

async function getData(id) {
  const items = data.filter((item) => item.surah.toString() === id);
  return items;
}

async function ItemPage({ params }) {
  const { id } = params;
  const items = await getData(id);

  return (
   <>
    <Navbar/>

   <SizeChanger />
<SizeChanger />
    <div className=" mt-6 items-center justify-center min-h-screen bg-gray-100 dark:bg-gray-500">
	<SizeChanger />
        {items.map((item, index) => (
          <div key={item.ayah}>
            <SurahCard 
              ayah={item.ayah} 
              surah={item.surah} 
              aerab={item.aerab} 
	      
              className="rtl mt-4" // Add RTL and font class
            />
            {index < items.length - 1 && <div className="border-b border-gray-300 dark:border-gray-700"></div>}

          </div>
        ))}
      </div>
   </>
  );
}

export default ItemPage;
